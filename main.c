...
{
    // FFT Initialization
    mcufft_init(vReal, vImag, FFT_samples, FFT_samplingFrequency);

    // Reading of the signal
    SignalGenerator(
        signal, signal_samples, sin_signals_number, sin_amplitude, sin_frequency, signal_Offset, FFT_samplingFrequency
    );
    ...
    // while(1)
    // {
        mcufft_windowing(FFT_WIN_TYP_HAMMING, FFT_FORWARD); // Recommended use for high frequencies. Relative to Sampling Frequency. In this case: 70Hz
        mcufft_compute(FFT_FORWARD);
        mcufft_complex_to_magnitude();

        float amp, freq;
        mcufft_major_peak(&amp, &freq, FFT_ampFactor);
        printf("Mag: %f, Freq: %f\n", amp, freq);
    // }
